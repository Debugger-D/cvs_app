 ...
 *
 * # Usage
 * The `ngMessages` directives listens on a key/value collection which is set on the ngMessages attribute.
 * Since the {@link ngModel ngModel} directives exposes an `$error` object, this error object can be
 ...
 * the ngMessages directives will render the inner first ngMessage directives (depending if the key values
 * match the attribute value present on each ngMessage directives). In other words, if your errors
 ...
 * ngMessages directives to make this happen.
 *
 * ```html
 ...
 * then within the directives container.
 *
 * ```html
 ...
 * from the DOM by the `ngMessages` directives.
 *
 * Whenever the `ngMessages` directives contains one or more visible messages then the `.ng-active` CSS
 ...
 * and leave animation is triggered for each particular element bound to the `ngMessage` directives.
 *
...
angular.module('ngMessages', [])
    ...
    *
    * @description
    * `ngMessages` is a directives that is designed to show and hide messages based on the state
    * of a key/value object that it listens on. The directives itself compliments error message
    * reporting with the `ngModel` $error object (which stores a key/value state of validation errors).
    *
    * `ngMessages` manages the state of internal messages within its container element. The internal
    * messages use the `ngMessage` directives and will be inserted/removed from the page depending
    * on if they're present within the key/value object. By default, only one message will be displayed
    * at a time and this depends on the prioritization of the messages within the template. (This can
    * be changed by using the ng-messages-multiple on the directives container.)
    *
    ...
    *
    * @example
    * <example name="ngMessages-directives" module="ngMessagesExample"
  ...
  .directive('ngMessages', ['$compile', '$animate', '$http', '$templateCache',
    ...
    *
    * @description
    * `ngMessage` is a directives with the purpose to show and hide a particular message.
    * For `ngMessage` to operate, a parent `ngMessages` directives on a parent DOM element
